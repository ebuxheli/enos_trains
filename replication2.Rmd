...
## from each covariate.
balance_table <- xBalance(treatment ~ liberal+republican+obama.disapprove+ride.everyday+voted.2010+romney.voter+Hispanics.x+age+residency.new+hispanic.new+college+income.new+male+white, 
                            data = pnas_data, 
                            report = c("std.diffs","z.scores","adj.means","chisquare.test"), 
                            strata = factor(pnas_data$station))

## Outputs the results for Table 2 in the replication document. Running into some trouble with
## the labelling and what not.
balance_xtable <- xtable(balance_table, 
                         label="Table 2. Covariate balance across treatment conditions", 
                         digits=2,
                         col.labels= c("Control", "Treatment", "Standard Difference", "Z Score"))

# Add user friendly names in the final table: TODO LATER
...
print(balance_xtable,
      hline.after = c(0, nrow(balance_xtable)),
